...
func watchAction(cmd *cobra.Command, args []string) error {
	...
	feedUrl := internal.GetCoinbaseWsUrl(viper.GetBool("coinbaseSandbox"))
...
func watchMessageHandler(message coinbasepro.Message) error {
	...
	case "ticker":
		...
		sugar.Infof("stored price of %s at %s", p.Pair, p.Amount)

		// determine average
		if average, err := models.PriceAverage(dbHandler); err != nil {
			return err
		} else {
			sugar.Infof("current price average is: %s", average)
		}
	case "heartbeat":
		sugar.Infow("new heartbeat message")
	case "error":
